...
const processMeta = (name, $post) => {
    ...
    if (/^(?:posts\/)?draft/.test(name)) {
        ...
        post.data.created_at =
            $post(".dt-published").attr("datetime") || dateNow;
...
const processFeatureImage = (html, post) => {
    ...
    if (featured) {
        ...
        post.data.feature_image_caption =
            $(featured).parents("figure").find("figcaption").html() || null;

        // Y: Don't remove the featured image from the
        // post content for better compatibility with Medium
